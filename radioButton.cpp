	...
	void check(int i)
	{
		if( i == 0) {
			if (checkBoxObj[0].getChecked() == false) {
				...
				COORD cLabel1 = { 1, 2 };
				HANDLE hLabel1 = GetStdHandle(STD_OUTPUT_HANDLE);

				SetConsoleCursorPosition(hLabel1, cLabel1);

				//set cursor size and visibility example

				CONSOLE_CURSOR_INFO cciLabel1 = { 100, FALSE };
				SetConsoleCursorInfo(hLabel1, &cciLabel1);

				cout << ' ';
				checkBoxObj[1].uncheck();

				COORD cLabel2 = { 1, 3 };
				HANDLE hLabel2 = GetStdHandle(STD_OUTPUT_HANDLE);

				SetConsoleCursorPosition(hLabel2, cLabel2);

				//set cursor size and visibility example

				CONSOLE_CURSOR_INFO cciLabel2 = { 100, FALSE };
				SetConsoleCursorInfo(hLabel2, &cciLabel2);
